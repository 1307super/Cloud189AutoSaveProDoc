...
## 一、媒体配置（转存增强功能）
### 1. OpenAI 配置（推荐[智普轻言](advanced/ai/#支持的-ai-平台)接口）
**功能说明**：  
...
- **TMDB识别**：调用TMDB接口获取影视详情（如海报、简介、演员信息等）。  
...
**全局开启**
- 在系统 `媒体` 页中 `STRM` 项勾选 `启用 STRM 生成`

> #### 如果你使用的是玄鲸极速模式，以下步骤可以直接忽略
**配置要点**：  
- **可选条件**：需在[账号管理](usage/account)中配置“[本地目录](usage/account)”（如`/天翼云盘`）。  
...
> 注意: 如果你没有全局的科学环境, 请务必在 `代理配置` 中配置有效的代理, 并勾选 `Telegram`, 否则无法使用机器人
